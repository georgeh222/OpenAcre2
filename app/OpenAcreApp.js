...
import { MapPin, Fish, Shield, CalendarCheck, DollarSign, Users, CheckCircle, Mail, Phone, LogIn, LogOut, Globe2, Clock, Ban, Map } from "lucide-react";
...
const faq = [
  ...
  { q: "What are the fees?", a: "Day-pass bookings include a 12% platform fee from the guest and a 5% owner fee, with a $2 minimum. See Pricing for memberships that can reduce fees." },
...
const TextInput = (props) => (
  <input {...props} className={`w-full rounded-xl border border-green-200 bg-white px-3 py-2 outline-none focus:ring-2 focus:ring-green-500/30 ${props.className||''}`} />
);
const TextArea = (props) => (
  <textarea {...props} className={`w-full rounded-xl border border-green-200 bg-white px-3 py-2 outline-none focus:ring-2 focus:ring-green-500/30 ${props.className||''}`} />
);
const Select = (props) => (
  <select {...props} className={`w-full rounded-xl border border-green-200 bg-white px-3 py-2 outline-none focus:ring-2 focus:ring-green-500/30 ${props.className||''}`} />
);
const Pill = ({ children }) => (
  <span className="inline-flex items-center gap-2 rounded-full border border-green-200 bg-white px-3 py-1 text-sm text-green-900/90">{children}</span>
);
...
function Landing({ onGoOwners, onGoHunters, listings, setListings }) {
  const [newListing, setNewListing] = useState({ name: "", lng: "", lat: "", species: "", pricePerDay: "" });
  const addListing = (e) => {
    e.preventDefault();
    const id = (newListing.name || "listing").toLowerCase().replace(/[^a-z0-9]+/g, "-") + Date.now();
    const next = {
      id,
      name: newListing.name || "New Listing",
      species: newListing.species ? newListing.species.split(",").map(s=>s.trim()) : [],
      pricePerDay: newListing.pricePerDay ? Number(newListing.pricePerDay) : 0,
    ...
    if (Number.isFinite(next.lng) && Number.isFinite(next.lat)) {
      ...
      setListings(updated);
      saveLocal("openacre_listings", updated);
      setNewListing({ name: "", lng: "", lat: "", species: "", pricePerDay: "" });
    }
  };
    ...
    <div>
      ...
      <div className="bg-green-50">
        <div className="max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="mt-6 text-green-900/80 max-w-xl">
              A clean, trusted marketplace connecting landowners and outdoorspeople for day-access.
              Simple rules. Clear boundaries. Instant bookings.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <button onClick={onGoOwners} className="rounded-xl bg-green-700 hover:bg-green-800 text-white px-5 py-3 font-medium inline-flex items-center gap-2">
                <Shield className="h-4 w-4" />I’m an Owner
              </button>
              <button onClick={onGoHunters} className="rounded-xl bg-white hover:bg-green-50 text-green-900 border border-green-200 px-5 py-3 font-medium inline-flex items-center gap-2">
                <Fish className="h-4 w-4" />I’m a Hunter
              </button>
            </div>
      ...
      <Section id="features" eyebrow="Why OpenAcre" title="Clean, safe access with rules that work" subtitle="Built like Airbnb—identity, reviews, clear fees, and reliable payouts.">
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { icon:<Shield className="h-5 w-5"/>, title:"Verified access", text:"ID, license & (where required) insurance verifications so owners can host confidently." },
            { icon:<Map className="h-5 w-5"/>, title:"Clear boundaries", text:"Map zones, hours, and no-go areas. Hunters see the rules before booking." },
            { icon:<CalendarCheck className="h-5 w-5"/>, title:"Instant bookings", text:"Open your calendar or approve requests—your choice." },
            { icon:<DollarSign className="h-5 w-5"/>, title:"Simple fees", text:"12% guest + 5% owner (min $2). Memberships can lower fees." },
            { icon:<Users className="h-5 w-5"/>, title:"Two-way reviews", text:"Ratings for hunters and owners build trust." },
            { icon:<CheckCircle className="h-5 w-5"/>, title:"Rule-first design", text:"Zones & Hours, No-Transfer, check-in/out photos—automated compliance." }
          ].map((f,i)=>(
      ...
      <Section id="experiences" eyebrow="Experiences" title="Browse access by species & season" subtitle="Add or edit locations anytime—this is your live map.">
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="rounded-2xl border border-green-200 bg-white p-5">
            ...
            <form onSubmit={addListing} className="space-y-3">
              <Field label="Name" required><TextInput value={newListing.name} onChange={e=>setNewListing(v=>({...v, name:e.target.value}))} required /></Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Longitude" required><TextInput value={newListing.lng} onChange={e=>setNewListing(v=>({...v, lng:e.target.value}))} placeholder="-82.75" required /></Field>
                <Field label="Latitude" required><TextInput value={newListing.lat} onChange={e=>setNewListing(v=>({...v, lat:e.target.value}))} placeholder="42.08" required /></Field>
              </div>
              <Field label="Species (comma-separated)"><TextInput value={newListing.species} onChange={e=>setNewListing(v=>({...v, species:e.target.value}))} placeholder="Whitetail, Turkey" /></Field>
              <Field label="Price per day ($)"><TextInput type="number" min={0} step={1} value={newListing.pricePerDay} onChange={e=>setNewListing(v=>({...v, pricePerDay:e.target.value}))} placeholder="85" /></Field>
              <button className="w-full rounded-xl bg-green-700 hover:bg-green-800 text-white px-4 py-2 font-medium">Add to Map</button>
            </form>
            <p className="text-xs text-green-900/60 mt-3">Tip: Owner page has full property details + rules.</p>
          </div>
        </div>
      </Section>
      ...
      <Section id="pricing" eyebrow="Pricing" title="Transparent fees—no surprises" subtitle="Choose pay-as-you-go or memberships for savings.">
        <div className="grid md:grid-cols-3 gap-6">
          <div className="rounded-2xl border border-green-200 bg-white p-6">
            ...
            <ul className="mt-4 space-y-2 text-green-900/80 text-sm">
              <li>• No monthly commitment</li><li>• Two-way reviews</li><li>• Basic support</li>
            </ul>
          ...
          <div className="rounded-2xl border-2 border-green-600 bg-green-50 p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-2 text-green-900/80 text-sm">
              <li>• Early booking windows</li><li>• 50% off guest fees</li><li>• Priority support</li>
            </ul>
          ...
          <div className="rounded-2xl border border-green-200 bg-white p-6">
            ...
            <ul className="mt-4 space-y-2 text-green-900/80 text-sm">
              <li>• 50% off owner fees</li><li>• Featured placement</li><li>• Dedicated payouts line</li>
            </ul>
      ...
      <Section eyebrow="For Landowners" title="Turn acres into income—without losing control" subtitle="Approve who comes on. Set rules and hours. Get paid automatically.">
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            { icon:<Users className="h-5 w-5"/>, title:"Vetted guests", text:"Hunters upload ID, license, and (if required) insurance." }
          ].map((c,i)=>(
            <div key={i} className="rounded-2xl border border-green-200 bg-white p-5">
              <div className="flex items-center gap-2 text-green-800 font-semibold">
      ...
      <Section title="Join the Owner waitlist" subtitle="We’ll notify you when onboarding opens in your region.">
        <form onSubmit={submit} className="grid md:grid-cols-2 gap-5">
          ...
          <div className="md:col-span-2">
            <label className="flex items-center gap-2 text-sm text-green-900/90">
...
function LegalPage() {
  ...
  const TabBtn = ({id, children}) => (
    <button onClick={()=>setTab(id)} className={`px-4 py-2 rounded-xl border ${tab===id?'bg-green-700 text-white border-green-700':'bg-white text-green-900 border-green-200'}`}>{children}</button>
  );
  ...
  return (
    <Section eyebrow="Legal" title="Our policies" subtitle="Placeholders—replace with counsel-approved text.">
      <div className="flex gap-2 mb-4">
        <TabBtn id="terms">Terms</TabBtn>
        <TabBtn id="privacy">Privacy</TabBtn>
        <TabBtn id="waiver">Waiver</TabBtn>
      </div>
      {tab==="terms" && <Block title="Terms of Service (Placeholder)"><p>Marketplace connecting owners and lawful guests. You agree to follow rules and applicable laws. Fees disclosed at checkout. Arbitration & liability limits apply as permitted by law.</p></Block>}
      {tab==="privacy" && <Block title="Privacy Policy (Placeholder)"><p>We collect account, verification, geo-boundary, and usage data to provide and improve the service. We don’t sell personal data. Deletion available subject to legal retention.</p></Block>}
      {tab==="waiver" && <Block title="Liability Waiver (Placeholder)"><p>You acknowledge inherent outdoor risks and assume them to the extent permitted by law. Follow Zones & Hours, No-Transfer, and check-in/out procedures.</p></Block>}
    </Section>
  );
}
...
export default function OpenAcreApp() {
  ...
  return (
    <div className="min-h-screen bg-white text-green-900">
      ...
      <footer className="border-t border-green-100 mt-14">
        <div className="max-w-6xl mx-auto px-4 py-10 md:py-12 grid md:grid-cols-4 gap-8 text-sm">
          <div><div className="text-lg font-semibold">OpenAcre</div><div className="text-green-900/70">Hunt &amp; Fish Everywhere.</div></div>
          <div><div className="font-semibold mb-2">Product</div><ul className="space-y-1 text-green-900/70"><li>Home</li><li>Pricing</li><li>FAQ</li></ul></div>
          <div><div className="font-semibold mb-2">Company</div><ul className="space-y-1 text-green-900/70"><li>About</li><li>Contact</li><li>Legal</li></ul></div>
          <div><div className="font-semibold mb-2">Legal</div><div className="text-green-900/70">Terms • Privacy • Waiver</div></div>
        </div>
